...
class KomiZyrianAnalyzer(Analyzer):
    def __init__(self, mode='strict', verbose_grammar=False):
        ...
        if mode not in ('strict', 'nodiacritics'):
            return
        self.dirName = 'uniparser_komi_zyrian.data_' + mode
    ...
    def analyze_words(self, words, format=None, disambiguate=False):
        ...
        # if disambiguate:
        #     with as_file(files(self.dirName) / 'komi_zyrian_disambiguation.cg3') as cgFile:
