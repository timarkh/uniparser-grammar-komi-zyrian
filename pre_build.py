...
def collect_lemmata(dirName):
    ...
    for fname in os.listdir(dirName):
        if fname.endswith('.txt') and fname.startswith('kpv_lexemes_'):
...
def prepare_files():
    ...
    Put all grammar files to ../uniparser_komi_zyrian/data_strict/
    (original version) or ../uniparser_komi_zyrian/data_nodiacritics/
    (diacriticless version).
    """
    lemmata, lexrules = collect_lemmata('.')
    fOutLemmata = open('uniparser_komi_zyrian/data_strict/lexemes.txt', 'w', encoding='utf-8')
    fOutLemmata.write(lemmata)
    fOutLemmata.close()
    fOutLemmataRus = open('uniparser_komi_zyrian/data_nodiacritics/lexemes.txt', 'w', encoding='utf-8')
    ...
    fInParadigms.close()
    fOutParadigms = open('uniparser_komi_zyrian/data_strict/paradigms.txt', 'w', encoding='utf-8')
    fOutParadigms.write(paradigms)
    fOutParadigms.close()
    fOutParadigmsRus = open('uniparser_komi_zyrian/data_nodiacritics/paradigms.txt', 'w', encoding='utf-8')
    fOutParadigmsRus.write(russify(paradigms))
    fOutParadigmsRus.close()
    fOutLexrules = open('uniparser_komi_zyrian/data_strict/lex_rules.txt', 'w', encoding='utf-8')
    fOutLexrules.write(lexrules)
    fOutLexrules.close()
    fOutLexrules = open('uniparser_komi_zyrian/data_nodiacritics/lex_rules.txt', 'w', encoding='utf-8')
    fOutLexrules.write(lexrules)
    fOutLexrules.close()
    shutil.copy2('bad_analyses.txt', 'uniparser_komi_zyrian/data_strict/')
    shutil.copy2('bad_analyses.txt', 'uniparser_komi_zyrian/data_nodiacritics/')
    # shutil.copy2('komi_zyrian_disambiguation.cg3', 'uniparser_komi_zyrian/data_strict/')
    # shutil.copy2('komi_zyrian_disambiguation.cg3', 'uniparser_komi_zyrian/data_nodiacritics/')
